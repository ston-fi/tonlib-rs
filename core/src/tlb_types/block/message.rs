...
mod tests {
    ...
    fn test_common_msg_info_int() -> anyhow::Result<()> {
        ...
        assert_eq!(TonAddress::from_tlb_int(info.src)?, expected_src);
        assert_eq!(
            TonAddress::from_tlb_int(info.dest)?.to_string(),
    ...
    fn test_ext_in_msg_info() -> anyhow::Result<()> {
        ...
        let dst = TonAddress::from_tlb(ext_in_msg_info.dest.clone())?;
