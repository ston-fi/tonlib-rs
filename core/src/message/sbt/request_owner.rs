...
impl TonMessage for RequestOwnerMessage {
    ...
    fn parse(cell: &Cell) -> Result<Self, TonMessageError> {
        ...
        let result = RequestOwnerMessage {
            query_id,
            dest,
            forward_payload,
            with_content,
...
mod tests {
    ...
    use crate::message::{HasOpcode, RequestOwnerMessage, TonMessage, WithForwardPayload};
    use crate::TonAddress;

    #[test]
    fn sbt_request_ownership_msg_test() {
        ...
        let expected = RequestOwnerMessage {
            query_id,
            dest: dest.clone(),
        ...
        let build_result = RequestOwnerMessage::new(dest, true)
        ...
        let parse_result = RequestOwnerMessage::parse(&cell);
